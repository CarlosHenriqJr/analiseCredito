...
class AnaliseCreditoSupabaseUser extends BaseAuthUser {
  AnaliseCreditoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> analiseCreditoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AnaliseCreditoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
